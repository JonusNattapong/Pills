...
name: dose-create
description: >
  Create a custom Dose pill. You describe the color, focus, and behavior —
  ...
  "new pill", "custom pill", "make me a pill", or invokes /dose-create.
---
...
This creates:
- `skills/dose-<name>/SKILL.md` — the pill's skill file
...
Append to `skills/dose/SKILL.md`:

Add a new row to the "How to pop" table:
```markdown
| `/dose <name>` / "pop <name>" <emoji> | **<Name>** | <color> | <focus> |
```
...
- Each pill gets its own `dose-<name>` skill folder
- The pill is immediately available via `/dose <name>` after registration
