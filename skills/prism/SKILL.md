...
description: >
  ...
  Use when the user says "prism", "pop red/blue/etc", any pill color, or
  invokes /prism. Each pill has its own ladder.
license: MIT
---
...
Say the color or use `/prism`:

| Command | Pill | Focus |
|---------|------|-------|
| `/prism red` / "pop red" 🔴 | **Titan** | Performance. Speed. Power. |
| `/prism blue` / "pop blue" 🔵 | **Sage** | Architecture. Patterns. Quality. |
| `/prism green` / "pop green" 🟢 | **Warden** | Security. Safety. Stability. |
| `/prism yellow` / "pop yellow" 🟡 | **Phantom** | Minimal. Ship. Less. Default. |
| `/prism purple` / "pop purple" 🟣 | **Void** | Full spectrum. All powers. |
...
- Mark perf shortcuts with `prism: perf — <ceiling>, <upgrade path>`.
...
- Mark architecture shortcuts with `prism: arch — <ceiling>, <upgrade path>`.
...
- `prism: security — <ceiling>, <upgrade path>` for intentional deferrals.
...
- `prism: phantom — <ceiling>, <upgrade path>`. The default and the safety net.
...
- Void does not have its own shortcuts — every `prism:` comment names the pill that deferred it.
